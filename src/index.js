  ...
  try {
    ...
    const handlerPromise = (async () => {
      ...
      if (interaction.type === InteractionType.APPLICATION_COMMAND) {
        console.error("Handling application command");
      ...
      if (interaction.type === InteractionType.MESSAGE_COMPONENT) {
        console.error("Handling message component");
      ...
      if (interaction.type === InteractionType.MODAL_SUBMIT) {
        console.error("Handling modal submission");
        return await handleModalSubmit(interaction, env);
      }

      console.error("Handling unknown interaction type");
...
async function handleCron(event, env, ctx) {
  ...
  try {
    ...
    if (error.message === "Cron timeout") {
      console.error(
        "Cron job timed out - this may indicate database performance issues",
      );
    }
  }
}
